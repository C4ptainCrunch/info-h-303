...
def show_bar(etablissement_id):
    ...
    return render_template('view_bar.html', bar=bar, e=bar.etablissement, tags=tags, comments=comments, commentForm=forms.Comment(), should_comment=(len(list(filter(lambda x:x.user_id == g.user.id and x.date == date.today(), comments))) == 0))
